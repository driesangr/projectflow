...
watch(includeTasks, async (val) => {
  if (val && tasks.value.length === 0 && props.storyId) {
    loadingTasks.value = true
    try {
      tasks.value = await listTasks(props.storyId)
...
async function handleDuplicate() {
  ...
  try {
    ...
    const newStory = await storiesStore.duplicate(props.storyId, taskIds)
    emit('duplicated', newStory)
  } catch (e: any) {
...
<template>
  <Modal :open="open" title="User Story duplizieren" @close="emit('close')">
    <div class="space-y-4">
      <ErrorBanner v-if="error" :message="error" class="mb-1" />

      <p class="text-sm text-gray-600">
        Eine Kopie von <span class="font-medium">{{ storyTitle }}</span> wird erstellt.
