...
import type { Bug, BugCreate, Sprint } from '@/types'
...
const form = reactive({
  ...
  status: props.initial?.status ?? 'todo' as Bug['status'],
...
<template>
  <form class="space-y-4" @submit.prevent="submit">
    <div>
      ...
      <input v-model="form.title" class="form-input" required placeholder="Bug title…" />
    </div>
    <div>
    ...
    <div>
      ...
      <textarea v-model="form.acceptance_criteria" class="form-textarea" placeholder="Steps to reproduce / expected behavior…" />
    </div>
    <div class="grid grid-cols-2 gap-3">
      ...
      <div>
        ...
        <input v-model.number="form.story_points" type="number" min="0" class="form-input" placeholder="e.g. 3" />
      </div>
    </div>
    <div class="grid grid-cols-2 gap-3">
      <div>
